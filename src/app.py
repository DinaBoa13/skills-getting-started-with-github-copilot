...
def unregister_from_activity(activity_name: str, email: str):
    ...
    if activity_name not in activities:
        raise HTTPException(status_code=404, detail="Activity not found")
    activity = activities[activity_name]
    # Remove all matching emails (case-insensitive)
    original_count = len(activity["participants"])
    activity["participants"] = [p for p in activity["participants"] if p.lower() != email.lower()]
    if len(activity["participants"]) < original_count:
